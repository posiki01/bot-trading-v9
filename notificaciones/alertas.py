#!/usr/bin/env python3
"""
notificaciones/alertas.py (V9.0 - REFACTORIZADO COMPLETAMENTE)
...
from utils.logger_persistente import get_logger
logger = get_logger()
...
class Notificaciones:
    ...
    def __init__(self,
        ...
        Args:
            ...
            almacen: Almacenamiento SQLite (para estadísticas)
        """
        ...
        # Comandos
        self.comando_callback: Optional[callable] = None
        ...
        self.logger.info(f"📢 Notificaciones V9.0 inicializadas")
        self.logger.info(f"   Discord: {'✅' if self.discord_webhook else '❌'}")
        self.logger.info(f"   Telegram: {'✅' if self.telegram_token and self.telegram_chat else '❌'}")
        self.logger.info(f"   Cola máxima: {self.MAX_QUEUE_SIZE}")
    ...
    def _worker(self):
        ...
        while self.running:
            try:
                # Timeout para permitir verificar self.running
            ...
            for attempt in range(1, self.RETRY_ATTEMPTS + 1):
                try:
                    ...
                    self.logger.warning(f"⚠️ Intento {attempt}/{self.RETRY_ATTEMPTS} falló: {e}")
                    if attempt < self.RETRY_ATTEMPTS:
                        time.sleep(self.RETRY_DELAY)
            
            if not success:
                self.logger.error(f"❌ Notificación fallida después de {self.RETRY_ATTEMPTS} intentos")
    ...
    def _enviar_mensaje(self,
        ...
        if self.discord_webhook:
            try:
                ...
                self.logger.warning(f"⚠️ Discord falló: {e}")
        ...
        if self.telegram_token and self.telegram_chat:
            try:
                ...
                self.logger.warning(f"⚠️ Telegram falló: {e}")
    
    def _crear_embed(self,
                     mensaje: str,
                     tipo: str = 'info',
                     titulo: Optional[str] = None,
                     campos: Optional[List[Dict]] = None,
        ...
        # Título
        if titulo is None:
            titulo = self.TITULOS.get(tipo, '📊 Información')
        ...
        embed = {
            ...
            'footer': {
                'text': footer or f'🤖 Bot Pepperstone | {datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")} UTC'
            }
        }
    ...
    def _enviar_discord(self, embed: Dict):
        ...
        try:
            ...
            if response.status_code not in [200, 204]:
                self.logger.warning(f"⚠️ Discord error: {response.status_code} - {response.text[:100]}")
                raise ValueError(f"Discord error {response.status_code}")
    ...
    def _enviar_telegram(self,
        ...
        if response.status_code != 200:
            raise ValueError(f"Telegram error: {response.status_code} - {response.text[:100]}")
    ...
    def enviar(self,
        ...
        try:
            ...
            self._stats['cola_rechazados'] += 1
            self.logger.warning("⚠️ Cola de notificaciones llena, mensaje descartado")
    
    def enviar_urgente(self,
                       mensaje: str,
                       titulo: Optional[str] = None,
                       tipo: str = 'error',
                       campos: Optional[List[Dict]] = None,
    ...
    def registrar_comando_callback(self, callback: callable):
        """
        Registra el callback para comandos remotos.
        """
        self.comando_callback = callback
        self.logger.info("📋 Callback de comandos registrado")
    ...
    def notificar_estado(self, estado: Dict[str, Any]):
        """
        Notifica el estado completo del bot.
        V9.0 - CORREGIDO: Recibe diccionario en lugar de objeto bot.
        """
        try:
            capital = estado.get('capital_actual', 0)
            pnl_hoy = estado.get('pnl_hoy', 0)
            win_rate = estado.get('win_rate', 0)
            total_ops = estado.get('total_operaciones', 0)
            etapa = estado.get('etapa', 1)
            posiciones = estado.get('posiciones', [])
            watchlist = estado.get('watchlist', [])
            modo = estado.get('modo', 'NORMAL')
            
            pos_texto = "📭 Sin posiciones abiertas"
            if posiciones:
                pos_lines = []
                for p in posiciones[:5]:
                    pnl = p.get('ganancia', 0)
                    emoji = "🟢" if pnl > 0 else "🔴"
                    pos_lines.append(f"{emoji} {p['simbolo']} | {p['direccion']} | ${pnl:+.2f}")
                pos_texto = "\n".join(pos_lines)
            
            watchlist_texto = "📭 Watchlist vacía"
            if watchlist:
                watchlist_texto = "\n".join([f"• {s}" for s in watchlist[:8]])
            ...
            self.logger.error(f"❌ Error en notificar_estado: {e}")
